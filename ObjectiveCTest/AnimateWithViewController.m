...
#import "AnimateWithViewController.h"

@interface AnimateWithViewController ()
...
@implementation AnimateWithViewController
...
-(void) initReplaceView{
    self.replaceView = [[UIView alloc] initWithFrame:CGRectMake(self.originX, self.originY, self.rectSize, self.rectSize)];
...
-(void) animationBtnClicked:(UIButton*)sender{
    [UIView transitionWithView:self.containerView duration:3.0 options:UIViewAnimationOptionTransitionCurlUp|UIViewAnimationOptionCurveEaseInOut animations:^{
        self.myView.frame = CGRectMake(self.originX + 50, self.originY, self.rectSize + 20,  self.rectSize + 20);
    } completion:^(BOOL finished) {
        self.myView.frame = CGRectMake(self.originX, self.originY, self.rectSize, self.rectSize);
    }];
}
/*
