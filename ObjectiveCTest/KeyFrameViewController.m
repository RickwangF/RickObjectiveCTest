...
-(void) initContaienrView{
    self.oriContainerX = self.view.frame.size.width / 2 - 50;
    self.oriContainerY = self.view.frame.size.height / 2 - 50;
    self.containerView = [[UIView alloc] initWithFrame:CGRectMake(self.oriContainerX, self.oriContainerY, 100, 100)];
...
-(void) animationBtnClicked:(UIButton*)sender{
    [UIView animateKeyframesWithDuration:8.0 delay:0 options:UIViewKeyframeAnimationOptionCalculationModeLinear | UIViewKeyframeAnimationOptionRepeat animations:^{
        
        [UIView addKeyframeWithRelativeStartTime:0 relativeDuration:0.25 animations:^{
            self.containerView.frame = CGRectMake(self.oriContainerX - 100, self.oriContainerY, 100, 100);
        }];
        
        [UIView addKeyframeWithRelativeStartTime:0.25 relativeDuration:0.25 animations:^{
            self.containerView.frame = CGRectMake(self.oriContainerX - 100, self.oriContainerY - 100, 100, 100);
        }];
        
        [UIView addKeyframeWithRelativeStartTime:0.5 relativeDuration:0.25 animations:^{
            self.containerView.frame = CGRectMake(self.oriContainerX, self.oriContainerY - 100, 100, 100);
        }];
        
        [UIView addKeyframeWithRelativeStartTime:0.75 relativeDuration:0.25 animations:^{
            self.containerView.frame = CGRectMake(self.oriContainerX, self.oriContainerY, 100, 100);
        }];
